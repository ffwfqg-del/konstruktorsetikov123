...
const nashivki = [
    ...
    {
        imageSrc: `${basePath}imgs/nashivki/ogleshenie.png`,
        stats: { neoglysh: 20 },
        ru_name: 'Нашивка на избежание оглушения',
        slot: 'armour', slot_name: 'Бронежилет'
    },
]
...
var selectedSkinYellowTransfer = null; // Хранит характеристики для переноса на скин
...
function updateStats() {
    ...
    if (selectedSkin) {
        ...
        if (selectedSkin != 'default') {
            // Используем перенесенные характеристики, если они есть, иначе оригинальные
            var yellow_skin = selectedSkinYellowTransfer || selectedSkin.yellow;
            
            // Если выбран скин Альтушка и есть перенос, сохраняем +100 к броне
            if (selectedSkin.ru_name === 'Альтушка' && selectedSkinYellowTransfer) {
                yellow_skin = { ...selectedSkinYellowTransfer };
    ...
    gridItems.forEach(item => {
        // Для бронежилета используется img.main, для остальных - img.main-accs
        const img = item.querySelector('img.main-accs') || item.querySelector('img.main');
        if (img) {
            // console.log(img)
...
function showSkinPereshiv() {
    var modalAccs = document.querySelector('.modal-accs');
    var modalOverlay = document.getElementById('modalOverlay');
    var modalTitle = modalOverlay.querySelector('h2');
    
    // Обновляем заголовок модального окна
    modalTitle.textContent = 'Выберите скин с которого будут взяты жёлтые характеристики';
    ...
    skins.forEach(skin => {
        let statsHtml = document.createElement('div');
        var mainText = document.createElement('div');
        mainText.textContent = skin.ru_name.replaceAll('"', '');
        mainText.style.marginBottom = '10px';
        statsHtml.appendChild(mainText);
        ...
        for (var key in skin.yellow) {
            if (RuTypes[key]) {
                var yellowElement = document.createElement('div');
                yellowElement.textContent = `${RuTypes[key]}: ${skin.yellow[key]}`;
                yellowElement.className = 'yellow';
                statsHtml.appendChild(yellowElement);
            }
        }
        ...
        var item_yellow = document.createElement('div')
        item_yellow.className = 'yellow_stats';
        item_yellow.setAttribute('data-yellow', JSON.stringify(skin.yellow));
        item_yellow.setAttribute('data-runame', JSON.stringify(skin.ru_name));
        item_yellow.appendChild(img)
        item_yellow.appendChild(statsHtml)

            item_yellow.addEventListener('click', function () {
            var transferYellow = JSON.parse(this.getAttribute('data-yellow'));
            var currentSkinName = selectedSkin.ru_name;
            
            // Если выбран скин Альтушка, всегда сохраняем +100 к броне от Альтушки
...
function createAndAppendImages(dataArray, containerSelector) {
    ...
    // Для бронежилета используется класс 'main', для остальных - 'main-accs'
    const isArmour = containerSelector.includes('#armour');
    mainImg.classList.add(isArmour ? 'main' : 'main-accs');
...
$(document).ready(function () {
    ...
    $('#save-set').on('click', function () {
        ...
        const buildSetFromDOM = () => {
            ...
            ids.forEach(id => {
                ...
                // Для бронежилета используется img.main, для остальных - img.main-accs
                const mainImg = item.querySelector('img.main-accs') || item.querySelector('img.main');
        ...
        (async () => {
            ...
            const skinYellowTransfer = selectedSkinYellowTransfer ? JSON.stringify(selectedSkinYellowTransfer) : null;
            const newSet = {
                id: crypto.randomUUID(),
                name: setName,
                skin: skin,
                skinYellowTransfer: skinYellowTransfer,
                ...buildSetFromDOM()
            };
    ...
    function renderSets(new_sets) {
        ...
        assets_sets.forEach((set, i) => {
            ...
            var armour_nashivka = getNashivkaInfo(armour.nashivka);
            ...
            const setsHtml = `
                <div class="modal-set"
                    ...
                    data-skinYellowTransfer='${set.skinYellowTransfer || ''}'
                ...
                >
                    ...
                    { type: 'armour', main: armour_main, yellow: armour_yellow, nashivka: armour_nashivka, zatochka: armour.zatochka },
        ...
        container.find('.modal-set').each(function () {
            ...
            $modalSet.on('click', function () {
                ...
                const skinYellowTransfer = $(this).data('skinYellowTransfer')
                if (skin) {
                    if (skin == '"default"') {
                        ...
                        // Восстанавливаем перенесенные характеристики скина, если они есть
                        if (skinYellowTransfer) {
                            try {
                                selectedSkinYellowTransfer = JSON.parse(skinYellowTransfer);
                            } catch (e) {
                                selectedSkinYellowTransfer = null;
                            }
                        } else {
    ...
    $('#closeModal').on('click', function () {
        ...
        if (slotName) {
            ...
            // Если это модальное окно переноса скинов, используем оригинальные характеристики
            if (selectedSkin && selectedSkin !== 'default') {
                selectedSkinYellowTransfer = null;
                updateStats();
            }
            // Восстанавливаем исходный заголовок модального окна
            var modalOverlay = document.getElementById('modalOverlay');
            var modalTitle = modalOverlay.querySelector('h2');
            modalTitle.textContent = 'Выберите аксессуар с которого будут взяты основные характеристики';
    ...
    $('#closeSkinModal').on('click', function () {
        $('#modalSkinsOverlay').fadeOut(200, function () {
            $(this).css('display', 'none');
            selectedSkin = 'default';
            selectedSkinYellowTransfer = null; // Сбрасываем перенос
            let randomElement;
    ...
    $('#closeXModalSkin').on('click', function () {
        $('#modalSkinsOverlay').fadeOut(200, function () {
            $(this).css('display', 'none');
            // Если был выбран скин, но не был выбран перенос, сбрасываем перенос
            if (selectedSkin && selectedSkin !== 'default') {
                selectedSkinYellowTransfer = null;
            }
            updateStats();
    ...
    $('.btn.nashivkabronik').on('click', function (e) {
        ...
        container.on('click', '.modal-nashivka', function (e) {
            ...
            // Для бронежилета может быть как img.main, так и img.main-accs
        ...
        // Всегда показываем модалку, но внутри обработчика просто не применим нашивку, если предмета нет
